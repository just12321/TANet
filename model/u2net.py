            ...
            with torch.enable_grad():
                if closure is not None:
                    ...
                    loss_total = F.binary_cross_entropy_with_logits(mask, x.float()) + \
                        sum([F.binary_cross_entropy_with_logits(h, x.float()) for h in hidden])
    ...
    def backward(self, x, optimer, closure=None, clear_stored=True):
        def wrap_closure(x, optimer):
            def closure_(pre):
                ...
                return loss
            return closure_
        return super().backward(x, optimer, wrap_closure, clear_stored)
...
class DeepLossWrap(LossWrap):
    ...
    def __call__(self, x, optimer):
        def _closure(pre):
            ...
            optimer.zero_grad()
            loss_total = sum([(
                                (v['loss'](mask, x.float(), **v['args']) + \
                               sum((v['loss'](h, x.float(), **v['args']) for h in hidden))) / (len(hidden) + 1)
                            ) * v.get('weight', 1) for _, v in self.losses.items()])
            loss['total'] = loss_total.item()
            loss_total.backward()
            optimer.step()
            return loss
